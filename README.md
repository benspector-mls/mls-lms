...
**An address that points inside the answer-key repository is understood as one.** Pasting `…/tree/main/answer-keys/mod-1-js-fundamentals/swe-1-2-strings-conditionals` — the address of the folder an instructor already has open — names the repository *and* the directory, so there is nothing left to search for and the keys under it are offered immediately. A `/blob/` link to a file resolves to the directory it is in, since a file path is not a directory. Two things it deliberately does not do: the path is **not** stored, because `answerKeyPaths` holds full repository paths and the column is a repository identity; and the branch in `/tree/main/` is **dropped**, because answer keys are read at the repository's default branch and a listing opened at some other branch would show files grading would not read. A root URL carrying no path is not a lesser answer — it is the right one for a repository holding a single assignment's solutions and nothing else.
...
- **`answerKeyRepo`** holds the reference solutions and must be private. `sections[].answerKeyPaths` are paths inside it, at any depth, exactly as they appear there. A public one is refused rather than warned about, because reference solutions readable by the students being graded against them is not a configuration detail.
...
**The answer-key repository is browsed, not typed.** This is the one part of the old catalogue machinery that still earns its keep, one level down: a mistyped answer key path is not an error anybody sees, it is a report written confidently without the reference solution it should have been compared against. So the authoring form walks the named repository directory by directory, and each section ticks from everything beneath the chosen one — recursively, because keys nest: `swe-1-3-node-modules` keeps its under `madlib-challenge/`.
...
A section naming no answer keys reads no second repository at all: nothing is resolved, nothing is requested, and the answer-key commit is null. Naming paths while the assignment names no repository is refused rather than skipped, because grading silently without reference solutions is the failure the whole mechanism exists to prevent.
