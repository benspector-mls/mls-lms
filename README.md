...
**Low confidence is a pill and not a flag.** It used to be both: a non-gating cross-check finding whose only effect was a badge, beside a confidence pill that says the same thing on every section always rather than conditionally. The finding is gone and `NON_GATING_FINDINGS` is now empty — kept as the seam a future non-gating finding belongs in, and as the place confidence would have to gate again if automatic approval is ever built. `FLAG_META` keeps its `LOW_CONFIDENCE` entry, because that map decodes *stored* flags and drafts written before the change have the code in their arrays; without it a raw `LOW_CONFIDENCE` string would render as a badge.
...
**Provisioning and the webhook.** `accept` creates a repository from the template with the student and instructors as collaborators; run a second time it reuses the repository rather than failing. The `classroom.yml` removal is **not** among the verified claims, and the distinction matters: no repository in `marcy-lms-test` has one, or any workflow at all, so `removeClassroomWorkflow` returns on a 404 every time it runs and has never removed anything. It is written for the templates that came from GitHub Classroom, which live in an organization this App is not installed on. Fixing those templates at the source is better than relying on this, and it is what makes the [asynchronous-copy race](ROADMAP.md#the-copy-is-asynchronous-and-that-is-a-bug-in-accept-today) harmless rather than lucky. A real pull request from `draft` into `main` fires the webhook, the signature verifies, and the submission becomes `SUBMITTED` with `isLate` computed. An invalid signature is rejected with a 401.
