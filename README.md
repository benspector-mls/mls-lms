...
| `npm run verify:cohorts`      | Cohorts, and that filtering to one narrows all four of a course's screens to the same set of fellows                                                                                             |
| `npm run verify:team-sets`    | Making team sets and placing fellows on teams, and what the database refuses on its own — a set is a partition, one row holds a team's work, and a mirror never points at another mirror          |
| `npm run verify:team-work`    | Work handed in by a team: any member hands in, the pile counts the team once, a released grade reaches every member identically, and each member's page reads their own record                    |
| `npm run verify:resources`    | Readings, notes, and videos — including every URL shape the video embed refuses                                                                                                                  |
| `npm run verify:enrollment`   | Starting a program, its roster and join link, creating and copying courses, publishing one, and the removed-fellow pair — through the callers                                              |
| `npm run verify:programs`     | The instructor link, who is named on which course, ownership in pairs, and deleting a program — through the callers                                                                        |
| `npm run verify:attendance`   | Starting a program day, the code and replacing one that got out, corrections, the backstop, ending and reopening, and the arrival averages — plus the two rules that live in Postgres            |
