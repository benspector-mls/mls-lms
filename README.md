...
**Authorization lives in exactly one place: procedure code.** `trpc/init.ts` layers `protectedProcedure` (a session), `profileProcedure` (a profile row), `studentProcedure`, `instructorProcedure` (`INSTRUCTOR` or `ADMIN`), and `adminProcedure`. Instructor procedures additionally check that the caller teaches *this* course rather than merely holding the role, because a role alone would let one cohort's instructor read another's.

### One way to ask each question

That check has exactly two shapes, and which one a procedure uses is decided by what its input names.

**`courseProcedure`, when the input already names the course.** It is `instructorProcedure` plus a `courseId` input plus the check, so a procedure built on it *cannot* omit the check — there is no line to leave out, the same way `protectedProcedure` gates a session. tRPC merges chained `.input()` schemas, so a procedure adding its own keeps `courseId` in its input type and no browser call site knows the difference. Around twenty procedures use it.

**The `teachable*` loaders in `lib/courses/scope.ts`, when the input names a row.** Middleware cannot serve these: a module id says nothing about which course it is in until the row is read, so loading and authorizing are one act. Each entity gets one — `teachableCourse`, `teachableModule`, `teachableAssignment`, `teachableSubmission`, `teachableEnrollment`, `teachableGroup`, `teachableResource`, `teachableDraft`, `teachableTestRun` — and each keeps choosing its own `select`, which is what lets the test-run panel ask for `runnerPreset` and the grading draft ask for `sections` without either having to agree with the other.
...
**The check is a condition in the `where`, not a join in the `select`.** That is what makes the loader one query rather than two, and it is deliberate in three ways worth knowing. The caller's `select` reaches Prisma untouched, so the returned payload type is exactly what they asked for with no cast and no merge — merging a probe into a caller's select is how a spread silently takes their columns with it, since an object literal has one `assignment` key. An admin has no condition added at all, so the common case pays nothing for a privilege it is not using. And `NOT_FOUND` is still told apart from `FORBIDDEN`, by a `count` that runs only when the first query returns nothing — on the failure path, where an extra round trip is free.

Three questions deliberately did not collapse into either, and each says why in its own comment: an instructor being refused enrollment as a student in their own course, reading a remembered `gradingGroupId` off the instructor row, and grading triage — which answers "not yours" with an empty pile rather than a refusal, because the two reasons to be on that screen with nothing on it are not worth telling apart.
...
**The `anon` and `authenticated` roles hold no privilege on any table in `public`, and a new table inherits none.** Those are the roles the browser's Supabase client connects as, so what this closes is querying a table directly from client JavaScript. It is not a restriction on people: every read and write a signed-in person performs goes through a procedure, which reaches Postgres as Prisma and owns these tables. Migration `20260814024306_revoke_public_grants_project_wide` subtracts `anon` and `authenticated` from Supabase's default privileges for every role that creates a table here, so a table added in a hurry is closed on creation rather than open. The block above is still written on each new table — the default is a property of the database and the block is a property of the table, and the one that travels with the table is the one a reader finds.
...
**`profiles`** carries the `Role` enum, `githubUsername`, a display name fallback, and `githubUserId BigInt? @unique`. The numeric ID is recorded by the `sync_github_identity` trigger from `auth.identities.provider_id`, guarded by a regular expression because that column is text and other providers put non-numeric values in it. Repository naming still uses the username, because that is the existing convention, which is why `submissions.repo_github_login_at_creation` exists. `display_name` is filled at signup by `handle_new_user` and is the one column on this table a person may change about themselves, through the `updateDisplayName` procedure — see [your own account](#your-own-account-and-the-name-a-roster-shows). What the `anon` and `authenticated` roles may write on this table is nothing, `role` and `github_username` least of all, so the only route to any of these columns is a procedure that decided the caller may take it. `verify:staff` asserts that the writable set is empty, because every procedure could be correct and a slipped grant would still let a student set their own `role` from browser JavaScript without going through one.
...
They live side by side in `lib/courses/membership.ts` because the first two `where` clauses differ by one enum value in code that otherwise reads identically. Written out at each call site, the failure is not spotting a difference — it is not noticing there was a decision to make. The third is beside them because it is the only place where owning something and teaching it grant the same thing, and it governs the *whole* of the access control on uploads: the bucket is private and carries no policies, so there is no other route to the bytes.
...
**The guarantee that is not in any procedure**: `anon` and `authenticated` hold no privilege on `profiles` or `instructor_invites`, so neither table can be read or written from client JavaScript at all — which is a statement about that route, not about what a person may do through a procedure. `verify:staff` asserts that the writable set on `profiles` is empty, because every procedure here could be perfect and a slipped grant would still let a student promote themselves from browser JavaScript, which is why `20260730024911_tighten_profiles_grants` exists and why `20260814024306_revoke_public_grants_project_wide` made it the default for every table rather than a thing to remember.
...
A published course cannot be checked from the outside. The Modules screen shows a course's shape and deliberately has nothing to press, so an assignment whose instructions make no sense, or whose kind hands out the wrong thing, was found by the first real student to reach it. A **test student** is a student-shaped identity an admin creates, looks through, accepts work as, and grades from the instructor side. Its submissions are ordinary rows and its repositories are ordinary repositories, which is the only arrangement in which a rehearsal catches anything.

**`Profile.testStudentNumber` is the whole of the data model** — one nullable integer, unique, whose presence says the profile is a test student and whose value is the number in its name. One column rather than a boolean beside a counter, so the two facts cannot disagree, and unique because that is also what makes two admins creating one at the same moment safe: the second loses the race at the database rather than by having checked first and been overtaken. `Test Student 3` carries the handle `test-student-3` and the address `test-student-3@test.invalid`, all three derived in `lib/students/test-student.ts`.
...
The cookie is unsigned and needs no signature, because it is never trusted — an admin later demoted stops being able to use it at their next request rather than at their next sign-in. Entering and leaving are route handlers rather than mutations, for two reasons: a mutation here cannot reliably write a cookie, and while the cookie is set the caller reads as a student, so an admin-guarded exit would refuse the one person entitled to press it. Leaving deliberately checks nothing at all. An amber bar across every screen names who is being looked through, because a rehearsal that looks like the real thing is a way to grade the wrong person.

**Leaving lands on the roster the admin switched in from**, which a second cookie records at that moment. It cannot be worked out at the point of leaving: a test student can be enrolled in several cohorts, and the question is not which one it is in but which one was being checked. The roster because that is the screen the View as button is on — leaving returns somebody where they left, and puts them a press away from switching in again, since checking a course takes more than one look. Two cookies rather than two values in one, because they carry different authority: the first is an entitlement, re-established from the database on every request; the second is a destination, whose worst failure is the wrong course's roster. It is validated as a uuid before it reaches a path all the same, since a path built from an unchecked cookie is how a redirect becomes somebody else's, and it is cleared rather than left behind when a switch names no course, so a stale value cannot send an admin back somewhere they were not. The fallback is `/instructor`.

**Accepting differs in exactly one respect: who is invited.** `test-student-3` names no GitHub account, so `PUT /collaborators/test-student-3` would answer 404 and fail an accept that had already created the repository. The admin looking through it is invited instead, with push, and is the account that pushes on the test student's behalf. The repository is private like every other, generated from the same template, named the same way. Everything downstream — the pull request, the test run, the report, the comment — is unaware, because there is nothing there to be aware of. Accepting is refused outright when there is no admin behind the test student or that admin has no linked GitHub account, which is stricter than the real-student path: there a missing instructor handle is a warning, because the student can work in their own repository regardless, and here it is the only account that will ever push.

Note that a *second* admin can push to a repository somebody else's preview created only if they are a listed instructor of that cohort, or the organization's base permission is Write. The ordinary answer is that each admin creates their own.

**One count excludes them and every list includes them.** `courses.listMine` leaves them out of the student count on a course card, which is the figure somebody quotes — a cohort of 25 must not read as 26 because a course was checked. The roster, gradebook, and triage list them like anybody, carrying a Test badge, because grading one is the point. The badge is one shared component precisely so the three screens cannot disagree; a screen that drew a test student as an ordinary name would show it as a student who has not started.

**Deleting one is wider than removing it from a cohort**, and both exist. Remove is the enrollment control every student has. Delete destroys the identity: its repositories are deleted from GitHub first, because the rows are what name them, and then the auth user goes and the profile cascades with every enrollment, submission, test run, and grading draft under it. It reaches every cohort the test student is in, which the confirmation names. A repository that fails to delete is reported rather than fatal — half-removed rows are worse than a leftover repository. `deleteRepo` is the only destructive GitHub call in the application and exists for this one caller.
...
**Which heading and which sample a section type takes comes from `lib/section-types.ts`**, which is also where its label on screen and the `Rubric` row it is graded against come from. One registry rather than four maps in four files, `satisfies Record<SectionType, …>` so a type added and forgotten is a compile error rather than a run that reads an undefined heading. It imports nothing and is browser-safe, because the section editor draws its picker from the same entries the prompt is built from.

Detection is deliberately not in it. `lib/grade/classify.ts` maps a changed file path to a section type through an *ordered* list where the first match wins — which is what decides that a flat `src/*.js` file is an algorithm exercise when the template has a Jest suite and frontend work otherwise — and keyed by type that order would become an accident of how the object literal happened to be written.

One thing the registry records rather than hides: `coding_sql` takes the frontend sample report, because the toolkit holds no SQL one. The sample teaches the *shape* of a report and the rubric heading supplies the criteria, so borrowing one is survivable in a way borrowing a rubric would not be — but it looks exactly like a copy-and-paste slip, so both the entry and a test say that it is not.
...
The three kinds handed in by link or by file — `GOOGLE_DRIVE`, `EXTERNAL_URL`, `FILE_UPLOAD` — can be corrected while the work is waiting. A student who pastes the instructor's template instead of their own copy, or uploads last week's draft, previously had no way back: the form was hidden the moment the work entered the queue, so the only route to a correct submission was to wait for a grade on work they already knew was wrong and then resubmit. `REPO` needs none of this, because pushing a commit is the correction.

**`handInMode` in `lib/status.ts` is the whole rule**, and it returns one of four acts rather than the two booleans it replaced. `submit` is work not yet handed in, and `ACCEPTED` counts — taking a copy of a Drive template is receiving the work, not returning it. `update` is a correction to work still waiting, which overwrites and leaves the submission exactly where it is in the queue. `resubmit` is a second attempt after a grade, which is the existing path below. `locked` is where an instructor has the work open. The gap that motivated it is visible in the shape: work in the queue matched neither old boolean, so the screen offered nothing and explained nothing.

**A student may not replace work an instructor is part-way through reading.** Handing in again is an overwrite — `submittedUrl` and the four upload columns are single-valued — so doing it while somebody is writing feedback would leave a grade describing a document nobody can open. The repository kinds are protected from exactly this by `draftIsStale`, which compares the draft's commit against the submission's; a link or a file has no commit, so the protection is instead a check for an open draft. It is deliberately narrow: `SUPERSEDED` is already replaced and `FAILED` produced nothing, so neither is anybody's work in progress, and blocking on them would lock a student out over a pipeline error they can neither see nor fix. An approved draft is not caught either, which is what leaves the ordinary resubmission path open.

**The check lives in `assertCanHandIn`**, so it holds for the tRPC mutation and the upload route alike — the same reason that function exists at all. The student's screen learns the same fact from a filtered `_count` on `assignments.listForCourse`, counting exactly the draft states the server refuses on, flattened to `instructorHasStarted` before it leaves the procedure. One number and no statuses: which state a grading draft is in is not a student's business, and "somebody is looking at this" is all the screen needs. The form and the mutation therefore cannot disagree about whether handing in is allowed, which matters because the failure mode is a student being offered a box that is then refused.
...
A submitted link is untrusted input in a way an uploaded file is not: the file goes to a private bucket this application controls, and the link goes wherever a student typed. `SubmittedLinkRow` therefore shows the address — the host drawn separately and first, the full URL underneath in a monospace face that wraps rather than truncates, because a URL is read left to right and a wrong one usually differs at the end. The ordinary mistakes are the common case and every one of them is obvious from the text and invisible behind a button: a Drive link ending `/template` is the instructor's own copy, a `localhost` address never had a chance of working.

**`linkHost` decides whether a link may become an anchor at all, and it accepts `http` and `https` only.** A URL is not the same thing as a web address — `javascript:alert(1)` parses perfectly well, and `submittedUrl` is a string a student typed that is rendered later on an instructor's signed-in page, which is the exact shape of a stored cross-site scripting hole. `data:` and `file:` are refused for the same reason. The `submitWork` schema refines on the same function, so nothing can be stored that the row would then refuse to open, and the row renders no `<a>` at all rather than a disabled one — a check that only greyed the button out would leave the href in the document. Where the address cannot be opened the row says so, because submissions predating the check can hold one.

**It sits beside `UploadedFileRow` on the review screen rather than inside the hand-grading card**, and that placement is the fix rather than a tidy-up: the card disappears the moment a draft exists, so an instructor who pressed "Start grading" lost the way to the document they were about to grade. The two components are the same fact about a submission for the two kinds that carry it, so they are the same shape in the same position. The student sees the row too, above the form that changes it, which is what lets them check their own link at the point a mistake is still free to fix.
...
**A mutation on an instructor screen refreshes twice, and `useServerMutation` is what does both.** There are two caches, and forgetting either leaves the screen wrong in a way that reads as the mutation having failed. The roster, the module list, and the gradebook are server components, so the browser holds no query to invalidate and only `router.refresh()` re-runs them; a group picker or a validation preview is the opposite, fetched through the tRPC client where `router.refresh()` does not reach. Most screens have both. The hook wraps a mutation's options rather than replacing them, so a call site keeps its own `onSuccess` for what is local to it — closing a dialog, naming what happened in a toast — and loses only the two lines it could get half right. Its `onError` defaults to putting the message in a toast; the two forms that print the refusal beside the field pass `shownInPlace`, so silence there is a decision rather than a swallowed error.

**`PageFallback` is the other one.** Cache Components requires every route to be a shell around a Suspense boundary, and that structure stays — what does not need writing fourteen times is the padded container around a `ListSkeleton` inside it. Its `width` matches what the real page renders, because the skeleton is replaced in place and a fallback wider than what follows makes the page jump sideways as it loads.
...
**The gradebook carries the same cells counted along both axes.** A Completed row under the assignment titles says how many students met the threshold on each — "2/5" — and a Completed assignments column before the work says how many each student has finished, "4/10". Both come from `lib/gradebook/summary.ts`, reading the payload the grid is already drawn from rather than a query of their own, which is what stops a total describing a different cohort than the rows beneath it.

**Complete means `isComplete` and never a score compared against a threshold**, here as everywhere: that judgment is made once, by `approveDraft`, in the same transaction that writes the status. The column is nullable, so the test is `=== true` — null is "no verdict yet", which is not failing and must not be counted as either. The two denominators differ on purpose: a column is measured against every student in the table, because an assignment nobody has attempted must read "0/5" rather than the "0/0" that counting only submissions would give; a row is measured against every assignment in the course, including any not yet handed out, so publishing something nobody has seen does not move a figure about work already done.

**A second column counts what is waiting on the instructor**, from `bucket != null` — which is exactly what draws the amber dot, so the number is the count of dots in its own row. A bare count rather than a fraction: "3 of 10" invites reading the ten as something to work through, where the useful figure is what is outstanding now. It is blank in the Removed students table, for the same reason the amber dot is suppressed there — that work is out of triage and nobody is going to grade it, so a count would claim a task that cannot be cleared.

**A legend above the grid names the three marks a cell carries when it has no score**, and they are one shape at three fills rather than three symbols: an empty ring, a grey dot, an amber dot, in the order work moves. The ring replaced an em dash, which was a typographic mark for "no value" standing among two pieces of interface and read as a different kind of thing rather than as the first step of a scale. The student's progress bar draws the same distinction the same way, outlined against filled. Fill against outline rather than two colours is also what keeps the pair legible to a reader who cannot tell the hues apart, and every mark carries its label as text.

**The legend takes its labels from `SUBMISSION_STATUS_META` and writes its own descriptions.** The labels because that map is the instructor's vocabulary everywhere else, so a cell's tooltip and the legend cannot drift; the descriptions because the map's are about repositories — "No repository created yet" — which is true of a `REPO` assignment and false of the other three kinds, and a gradebook mixes kinds freely.

**Each table summarises only itself**, so the Removed students table's figures describe removed students and contribute nothing to the cohort's. That is the same rule the two tables already exist for. The per-student column is deliberately not a second frozen column: only one thing can be pinned to the left edge, and pinning a second means hand-computing its offset from a first column whose width the table does not fix — a long name would push them apart and leave a gap that scrolls.

**The grading queue opens on All.** The tabs are All, To do, Graded, and the first is the one selected, because a leftmost tab that is not selected reads as a control somebody has already touched. The queue answers "how is this cohort doing on this piece of work"; "what do I do next" is a click away, and is what [triage](#triage) already answers a cohort at a time.

### Your own account, and the name a roster shows

`/profile` is the one screen in the signed-in application that belongs to a person rather than to a cohort, which is why it sits at the top level and why a student and an instructor reach the same address. It is reached from the account menu at the foot of the sidebar, beside the name and email it changes, and from nowhere else — everything in the navigation above that menu is a place to work, and this is the account those places are worked in. There is no role gate on the route because there is nothing for one to protect: `me` and `updateDisplayName` are both scoped to `ctx.user.id`, so the only profile the address can show or write is the caller's own.

**The display name is the only editable field, and it earns the screen.** Every account arrives with one the signup trigger derived — a GitHub profile's full name where there is one, and otherwise `split_part(email, '@', 1)` — so a cohort's roster opens reading `bspector`, `amina.k`, `jrivera23`. That string is also the gradebook's column of students, the grading queue's heading, and the sentence naming whose work is being read, so leaving it uncorrectable meant a term of grading people by the left-hand side of their email address.

**`displayNameSchema` in `lib/people.ts` is one definition read by both sides**, the form and the procedure, for the reason `lib/assignments/spec.ts` holds one definition of an assignment: a limit written out twice eventually disagrees with itself, and this particular disagreement surfaces as a field that accepts a name and a server that then refuses it. `.trim()` runs before the length checks, which is what makes `"  "` too short rather than two characters long and stops a pasted trailing newline from putting a maximum-length name one over. The ceiling is also a `maxLength` on the input, so it stops the typing instead of refusing the save, and the floor of two is there because `initials` draws a single letter either way — a one-character name costs a reader everything and gains them nothing. There is no way to clear a name, because the trigger fills the column for every account and reaching an empty one would mean asking to be listed on a roster as an email address.

**The rest of the screen is read-only and says where each value comes from**: the email is what Supabase authenticates and this application does not change it, the GitHub login is recorded by `sync_github_identity` and is fixed once a repository has been named after it, and the role is an admin's decision on the Staff screen. A disabled row that does not say what sets it invites exactly the question the sentence under it answers.

**A card lists what the application stores about a person, and what it does not.** Four columns, the cohorts and groups somebody is in, the work they hand in, and their grades and feedback — and then, said outright, no date of birth, address, phone number, government identifier, or payment detail. It is hard-coded rather than generated from the schema on purpose: a derived list would grow a row the moment a column was added, which sounds like an improvement and is the opposite, because the value of the card is that somebody decided each line belonged on it.

**Under an admin's test-student view this screen is the test student's**, because `createTRPCContext` substitutes the id for the whole request and every procedure behind the screen is scoped to it. Saving therefore renames the test student, which is a reasonable thing to want and not the same thing as renaming yourself — so the card says so in as many words, above the amber banner that is already on screen. See [seeing a course as a student sees it](#seeing-a-course-as-a-student-sees-it).
...
Every claim below was checked against real repositories in the `marcy-lms-test` organization, not asserted from reading the code. The re-runnable parts are `npm test` and the `verify:` scripts in [Scripts](#scripts); what remains outstanding is in [ROADMAP.md](ROADMAP.md).

The counts quoted below are what each script reported when its section was written. **`scripts/verify/BASELINE.md` is the number to compare a run against**, not these — a script grows and the prose does not, and two of the figures here had already fallen behind before that file existed.
...
**What the variance means, stated rather than smoothed over.** A single calibration run is not a measurement — this is the same submission, the same prompt, and the same model producing 12/15 and 13/15 on different days, so any future comparison between tiers has to run more than once or it is comparing noise. It does not undermine the pipeline: the cross-check passed on every run, the confidence rating was reported honestly, and the disagreement is one point on one question inside a band an instructor is expected to adjust. It does mean **the number to quote is a range, not a figure.**
...
**Test students.** `verify:test-student` takes `--live` and `--github` flags that each add a further group of checks. Two groups carry the weight. `enroll` and `remove` refusing a profile that is not a test student is the entire difference between the feature and a mutation that puts anybody in any course and deletes anybody's account with every grade they were given — checked against a real student, against the admin's own account, and for each of the five procedures against an instructor who is **not** also an admin. That last qualification is the group's own history: written as `role: { in: ["INSTRUCTOR", "ADMIN"] }` it selected the admin on a deployment whose only instructor is one, and three checks passed by asserting that the person who is allowed is allowed. And the switch is checked from both ends at once — the same cookie value permitted for an admin and refused for a student — because a substitution that works says nothing about the check that permits it.

The rest: a marked profile drops out of the course card's count in the same read that leaves it in the roster; accepting is refused with no admin behind it and refused again when that admin has no linked GitHub, while a real student's own missing-GitHub refusal is asserted beside them, since a mistake in the new branch would most likely show up as the old one no longer firing. `--live` creates a real account and deletes it, and **claims an account abandoned by a create that failed halfway** — a case that happened while this was being written, and whose residue is invisible in the worst way: an address registered so its number can never be used, and a profile reading as an ordinary student. `--live --github` generates a real repository, then asserts it is private, that the admin can push to it, that the test student's own handle was never sent to GitHub, and that deleting the test student takes the repository with it. **The last check of every run is that the run left no test student behind**, which is there because one did: cleanup that does not finish prints every `ok` it earned and leaves an account holding a number, a submission, and a repository, so the script now reports its own litter rather than leaving it to be found by hand.
