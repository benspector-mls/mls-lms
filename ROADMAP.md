...
- **`answerKeyRepo` is a column, "owner/repo", and must be private.** A public one is refused rather than warned about: reference solutions readable by the students being graded against them is not a configuration detail. `sections[].answerKeyPaths` are paths within it, at any depth. **A pasted address that points inside the repository opens the listing there**, so the folder an instructor already has open is one paste rather than a paste and then a walk back to the same place — the path is not stored, and the branch is dropped, since answer keys are read at the default branch.
