...
- **An early-intervention dashboard.** Each half of the signal now exists on its own screen: the attendance drift list says who is missing mornings, the arrival averages say who arrives late, and the gradebook's "Needs a conversation" list says who has slipped on the work, by a recent-window rule in `lib/gradebook/summary.ts` that the fellow's record also prints per course under Trends. What is left is joining them at the program level, so that a fellow who has stopped handing work in *and* stopped arriving on time surfaces once rather than on three screens. The fellow's Trends section is where that grows first, across attendance, assignments, projects and assessments, and it becomes a tab of its own if it gets large; what it should show is to be worked out after a term of reading the per-course list, and the program-wide read across every course is not built until then.
