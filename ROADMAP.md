...
It is grouped by the kind of work an item is: measurement, the instructor's grading hour, the fellow's side of the application, authoring assignments, the grading pipeline, running a program, and Salesforce. Each group ends with a **Deferred** list — items in that area that are deliberately not scheduled, several of which the database schema already leaves room for. [The order of work](#the-order-of-work) reads the same items in the order they should be built, which cuts across the groups.
...
- [The instructor's grading hour](#the-instructors-grading-hour)
  - [Built: reading the changed files without leaving the review](#built-reading-the-changed-files-without-leaving-the-review)
...
- [Running a program: enrollment and attendance](#running-a-program-enrollment-and-attendance)
  - [The GCF, and what is left of it](#the-gcf-and-what-is-left-of-it)
  - [Deferred: running a program](#deferred-running-a-program)
...
**Nothing about running a program needs manual entry in the database any more.** A program can be started, filled from one join link, instructed by several people, divided into cohorts, and retired; a course inside it can be created, copied from a previous year's, published, divided into teams, archived, found again afterwards, and finally deleted. Assignments of all four kinds can be authored, published, handed out — to each fellow or to a team — handed in by any member of one, graded by the pipeline or by hand, and released to everybody who did the work, and readings, notes, and videos sit under a module beside its work. Attendance is taken once a day for the whole program, and when people arrive is reported beside whether they came. Somebody can be made staff by an admin and added to a program by whoever runs it. The first admin of a deployment is still a hand-edited row, necessarily, because there is nobody to grant it — `npm run grant:admin` is that base case as a tool.
...
3. **[Notes a student keeps on their own work](#notes-a-student-keeps-on-their-own-work)** — the assignment panel's third tab, and a screen listing them. High for how little it is: one table, no new dependency, and it is the half of [the dashboard](ARCHITECTURE.md#what-is-due-across-every-course) that is about what a fellow took from the work rather than about what is left of it.
...
7. **[Targeted assignments, and excusing a student](#targeted-assignments-and-excusing-a-student)** — half of which is settled, since [a cohort](ARCHITECTURE.md#cohorts-and-grading-a-portion-of-a-roster) is the way to name a subset of a roster.
...
Figures a real program produces, and the parts of the working system a review deliberately left alone. None of it adds surface area, and all of it changes what the rest of this file should say.
...
**Questions to hold rather than work to schedule.** Nothing here is a known problem — the largest thing this has run against is one program — and most of what would answer it is measurement [token management](#token-management) produces anyway. It is written down because the answers change what [triggering and orchestration](#triggering-and-orchestration) should be, and that decision is already waiting.
...
**Supabase.** The application connects through the pooled `DATABASE_URL` and migrations use `DIRECT_URL`, which is the arrangement that survives many concurrent functions — a serverless fan-out against a direct connection is how a connection pool gets exhausted. Two other limits to know: the storage bucket for uploaded submissions grows, since a re-upload writes a new object and [the previous one is kept once a grade describes it](ARCHITECTURE.md#handing-in-a-file), and a hundred students' resumes at up to 25MB is a real number. `npm run reconcile:uploads` removes the objects nothing points at; the ones a grade describes are kept on purpose and are what accumulates.
...
**The one that is not a vendor limit.** A hundred fellows produce a hundred drafts an instructor has to read, and no amount of concurrency helps with that. Triage, [working a pile by what it is](#working-a-pile-by-what-it-is-not-only-by-what-it-needs), and [cohorts](ARCHITECTURE.md#cohorts-and-grading-a-portion-of-a-roster) are the parts of this application that actually address a roster of a hundred, which is worth noticing given how cheap they are.
...
- **It has to work across assignments**, which is the whole point — triage is already course-wide, so this belongs there rather than on one assignment's queue, and the queue's own filter should probably learn the same axis for consistency.
- **A way to work the filtered set in order.** Grading twenty resubmissions means opening one, approving it, and wanting the next one without going back to a list. The review surface has no next-and-previous today, and a filter that hands somebody twenty items and no way to walk them is half the feature. This is the part [cohorts](ARCHITECTURE.md#cohorts-and-grading-a-portion-of-a-roster) do not supply: a cohort narrows the four screens that already exist and needs nothing new to move between submissions, where working one pile of twenty at a sitting does.
...
- **Cost, and it is a different shape.** A term of one student's feedback is a large input for a single call, and it is per student rather than per section — so a roster of twenty-five is twenty-five long prompts with nothing shareable between them, since each one *is* a different student. Nothing here caches the way [an assignment's queue does](ARCHITECTURE.md#generating-every-pending-report-at-a-sitting).
- **Which course, and whether a fellow has more than one.** `submissions.listForStudent` already returns the other courses a fellow is in, because somebody repeating a program has two sets of work. A growth evaluation that silently covered one of them would be answering a narrower question than it appeared to.
...
The fellow's side of the application, beyond the course page and the [cross-course dashboard](ARCHITECTURE.md#what-is-due-across-every-course) that exist today.
...
**Half of the data-model decision is made.** Naming a subset of a roster was the missing piece and [a cohort](ARCHITECTURE.md#cohorts-and-grading-a-portion-of-a-roster) is it — a named division of one program's roster, carrying no instructor relation and no permission, which is exactly the shape targeting wants. So a targeted assignment is an assignment pointing at a cohort, with All Fellows — no cohort — the default it already behaves as. What it would cost is a course-scoped assignment naming a program-scoped set, which is the one composite key this change did not need to add.
...
**What is left is durability across a closed tab.** The fan-out is driven from the browser, so closing it stops what has not started — fine for a fellow's four assignments, a real limit for a whole roster. That is the remaining argument for a job table, and it is a smaller one than it was: four of the five requirements are already satisfied by rows that exist, so what a durable design would add is the ability to walk away, not correctness.
...
## Running a program: enrollment and attendance

Getting people onto a roster, recording who arrived and when, and the one assessment a fellow sits outside this application. The built versions are described in [getting fellows into a program](ARCHITECTURE.md#getting-fellows-into-a-program) and the attendance screen; everything below is what they do not do yet.
...
- **The target is program-wide and hard-coded** — 389 proctored, 600 on a mock's 1200 — in `GCF_TARGET` in `lib/gcf.ts`. That is right while it is a Marcy standard. It becomes wrong the day two programs are held to different bars, and the change then is a column on `Program` rather than a constant.
- **Nothing reads the trend.** Best and latest sit side by side on both screens, which is enough to see that somebody has stopped improving; a fellow sitting four mocks over a term has a shape that neither figure shows. Worth a sparkline before it is worth anything cleverer, and worth neither until a full program has practised through one.

### Deferred: running a program
...
- **An early-intervention dashboard.** `lastActivityAt`, `isLate`, and `status` already support it, and attendance now supplies two halves of the signal rather than one — the drift list says who is missing and the arrival averages say who is late. What is left is joining them to the work, so that a fellow who has stopped handing work in *and* stopped arriving on time surfaces once rather than on three screens.
- **Term dates, and a meeting pattern on a program.** The one thing that would let a morning nobody took attendance on be noticed. A session exists because somebody started one, so a forgotten Tuesday is indistinguishable from a Tuesday the program did not meet — and no warning can tell them apart without knowing which days are school days, which is why none is shown. `startsOn`, `endsOn`, and the weekdays a program meets would make the absent session detectable and the denominator "days we should have met" rather than "days somebody opened". It would also give the arrival averages a denominator they do not have: today they report the mornings somebody checked in and nothing about the ones they should have. Deferred because every version of it is a settings screen that has to be kept true, and being wrong about it is worse than being silent — and because `Program.term` is deliberately free text, so nothing is parsed out of "Fall 2026" today.
- **A second check-in later in the day**, and per-fellow modality — whether somebody dialled in on a day the program was in the building. Both are one column and neither is asked for yet; `@@unique([programId, date])` is what a second session per day would have to become `[programId, date, slot]`.
- **Copying a whole program.** `programs.create` copies nothing, so setting up next year is a new program and then `courses.create` once per course. That is four actions rather than one, and the version that fixes it is the existing per-course copy called in a loop — no schema change, and deliberately not built until somebody has rolled a year over by hand and can say what should carry across.
- **Adding a fellow to a roster directly, without the link.** It needs a way to find a person by email across the whole application, which is a search over `Profile` that nothing else needs and that exposes who else uses the system. The link and [the roster](ARCHITECTURE.md#getting-fellows-into-a-program) together cover the case that actually happens at the start of term: the instructor already writes down who is expected, and adding somebody mid-term is one more line in that box.
- **Renaming a program, or moving its term.** Neither is possible: the pair is what tells one year from another everywhere in the application and in the name of every exported file, and a program renamed in March would leave every reader of an older CSV holding a name that no longer exists. A program made by mistake is created again, which is cheap because it is created empty.
