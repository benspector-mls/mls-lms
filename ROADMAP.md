...
8. **[Targeted assignments, and excusing a student](#targeted-assignments-and-excusing-a-student)** — half of which is settled, since [a group](ARCHITECTURE.md#groups-and-grading-a-portion-of-a-cohort) is the way to name a subset of students.
9. **[AI grading for non-coding assignments](#ai-grading-for-non-coding-assignments)** — which begins with [instructor-authored rubrics](#instructor-authored-rubrics-are-a-prerequisite-not-a-companion), since none of the four fixed section types fits a resume or a reflection.
10. **[An evaluation of one student's growth across a term](#an-evaluation-of-one-students-growth-across-a-term)** — last of the grading-side features, and not because it is least wanted. It reads a term of released feedback, so it has nothing to read until a term has been graded, and it is the one feature here whose output is about a person rather than a piece of work.
11. **[A chat scoped to a student's own course context](#a-chat-scoped-to-a-students-own-course-context)** — last, and behind the item above it for a reason that is not sequencing: the two contradict each other on whether a synthesis across a term may reach a student unedited, and that has to be settled before either is built. It is also the one item here that needs a graded term before it can be *judged*, not merely before it can be run.
...
The data is already being fetched and thrown away. `getPullRequestFileChanges` in [lib/github/prs.ts](lib/github/prs.ts) calls the pull request's `files` endpoint, which returns a `patch` per file — a unified diff, already computed, already paid for — and the mapper keeps `path`, `kind`, and `previousPath` and drops it. Grading only ever needed to know which paths were present, so that was right. Showing the work needs the part being discarded.

What this touches, so the size is not a surprise:

- **The diff comes from the same call that already runs**, so there is no new round trip for the common case. GitHub omits `patch` on files it considers too large and on binaries, so "no diff available, open it on GitHub" is a real state rather than an error — and a renamed file with no content change has an empty one.
- **Whole-file contents are a second question.** A diff shows what changed; grading a submission often means reading a function that did not. `fetchChangedFiles` in [lib/grade/generate-report.ts](lib/grade/generate-report.ts) already reads file contents at a commit with a size cap, so the mechanism exists — but it is one request per file, which is fine for the handful a section touches and wrong as a default for a browser.
- **`promptExclusionReason` is not the right filter here, and reaching for it would be the easy mistake.** That list decides what must never be sent to a *third party* — a committed `.env` is unrecoverable once it is in somebody else's logs. An instructor reading a student's committed secret is the opposite: it is exactly what they need to see to tell the student to rotate it. What the filter is still right about is bulk — a committed `node_modules` is thousands of files nobody scrolls — so the shape is probably "collapsed by default, with the reason, and expandable", not "withheld".
- **It lands in [components/instructor/grading-review.tsx](components/instructor/grading-review.tsx)**, which is already the largest component in the repository and the one genuine split candidate the [review pass](#what-the-review-pass-left-open) left open. Adding a file browser to it without splitting it first is how that becomes permanent.
