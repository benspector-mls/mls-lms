...
async function main() {
  ...
  const { parseVideoUrl, resourceColumns, resourceSpecSchema, videoEmbedUrl, videoWatchUrl } =
  ...
  const recognised: [string, string, string][] = [
    ...
    [
      "a share link with the tracking parameter",
  ...
  */
  check(
    "YouTube: a timestamp does not stop it being recognised",
  ...
  check(
    "Vimeo: an unlisted link keeps the numeric id",
  ...
  */
  const refused: [string, string][] = [
    [
      "a host that merely contains youtube.com",
  ...
  */
  check(
    "refused: a traversal in place of an id",
    parseVideoUrl("https://www.youtube.com/embed/../../evil"),
    null,
  );
  ...
  check(
    "a YouTube embed is the no-cookie player",
    videoEmbedUrl({ provider: "YOUTUBE", videoId: "dQw4w9WgXcQ" }),
    "https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ",
  );
  check(
    "a Vimeo embed is the player host",
  ...
  */
  check(
    "the watch link is rebuilt from the id, not the paste",
  ...
  check(
    "a link needs a URL",
    resourceSpecSchema.safeParse({ kind: "LINK", title: "MDN" }).success,
    false,
  );
  check(
    "...and refuses something that is not one",
    resourceSpecSchema.safeParse({ kind: "LINK", title: "MDN", url: "mdn" }).success,
    false,
  );
  check(
    "a note needs a body",
    resourceSpecSchema.safeParse({ kind: "TEXT", title: "Notes", body: "  " }).success,
    false,
  );
  check(
    "every kind needs a title",
  ...
  check(
    "a description is optional and defaults to null",
    parsedLink.kind === "LINK" ? parsedLink.description : "wrong kind",
    null,
  );
  /*
  ...
  */
  check(
    "a note carrying a URL is refused rather than trimmed",
  ...
  */
  ...
  check(
    "a video stores its provider and id",
    [asVideo.videoProvider, asVideo.videoId],
    ["YOUTUBE", "dQw4w9WgXcQ"],
  );
  check(
    "...and a canonical watch link rather than the paste",
    asVideo.url,
    "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
  );
  ...
  check(
    "an unrecognised video is refused at the column boundary too",
    await refusal(async () =>
  ...
  try {
    await db.$transaction(
      ...
      */
        check(
          "a student sees the same resources an instructor does",
          (await asStudent.resources.listForCourse({ courseId: course.id })).length,
          listed.length,
        );
      ...
      */
        check(
          "changing a kind clears the old kind's columns",
          [retyped.kind, retyped.body],
          ["LINK", null],
        );
      ...
      */
        ...
        */
        ...
        check(
          ...
          await refusal(() => asStudent.resources.remove({ resourceId: video.id })),
          "FORBIDDEN",
        );
      ...
      */
        ...
        if (outsider) {
          ...
          check(
            "...nor remove one",
            await refusal(() => asOutsider.resources.remove({ resourceId: video.id })),
            "FORBIDDEN",
          );
        } else {
        ...
        // --- removing --------------------------------------------------------------
        check(
          "a resource can be removed",
          (
            await asInstructor.resources.remove({
              resourceId: video.id,
            })
          ).title,
          "Verify Mango",
  ...
  */
  ...
  check(
    "no resources survived the rollback",
    await db.resource.count({ where: { title: { startsWith: "Verify " } } }),
    0,
  );
  check(
