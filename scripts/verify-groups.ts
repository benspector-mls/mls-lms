...
async function main() {
  ...
  try {
    await db.$transaction(
      ...
      */
        ...
        check(
          "the group reports its member count",
          listed.groups.find((row) => row.id === squad.id)?.memberCount,
          1,
        );
      ...
      */
        ...
        check(
          "...and setting the same members again changes nothing",
        ...
        check(
          "...and a longer list replaces the membership",
          (await asInstructor.groups.listForCourse({ courseId: course.id })).groups.find(
            (row) => row.id === squad.id,
          )?.memberCount,
          2,
        ...
        check(
          "a student can belong to two groups",
          both.find((row) => row.enrollmentId === inside.id)?.groupIds.length,
          2,
        );
        ...
        check(
          "triage filtered to a group holds only that group's students",
          groupTriage.submissions.every((row) => row.student.id === inside.studentId),
          true,
        );
        check(
      ...
      */
        check(
          "...and the group plus everyone else is the whole pile",
        ...
        check(
          ...
          groupTriage.gradedCount <= allTriage.gradedCount,
          true,
        );
        ...
        check(
          "the gradebook lists only the group's students",
          groupBook.activeEnrollments.map((row) => row.student.id),
          [inside.studentId],
        );
        /*
      ...
      */
        check(
          ...
          groupBook.cells.every((cell) => cell.studentId === inside.studentId),
          true,
        );
        check(
          "...where unfiltered holds at least as many",
          allBook.cells.length >= groupBook.cells.length,
          true,
        );
        ...
        check(
          ...
          groupList.assignments.length,
          allList.assignments.length,
        );
        check(
          "...and never counts more work than the whole cohort has",
      ...
      */
        ...
        if (assignment) {
          ...
          check(
            "the queue lists only the group's students",
            queue.submissions.every((row) => row.student.id === inside.studentId),
            true,
          );
          check(
            "...and keeps an out-of-group submission openable, saying why",
        ...
        check(
          "Ungrouped excludes anybody who is in a group",
          ungrouped.submissions.every((row) => row.student.id !== inside.studentId),
          true,
        );
        check(
        ...
        check(
          "no filter and the All Students value are the same read",
      ...
      */
        ...
        if (elsewhere) {
          ...
          check(
            "a group from another course matches nothing rather than everything",
            (await asInstructor.submissions.triage({ courseId: course.id, group: foreign.id }))
              .submissions.length,
            0,
          );
          check(
            "...and cannot be remembered as this course's filter",
            await refusal(() =>
        ...
        check(
          "a removed student keeps their membership",
        ...
        check(
          "...and is out of the group's pile all the same",
          (
        ...
        check(
          "...and cannot be added to another group while removed",
          await refusal(() =>
        ...
        check(
          "restoring puts them back in the group they were in",
          (await asInstructor.groups.listForCourse({ courseId: course.id })).groups.find(
            (row) => row.id === squad.id,
          )?.memberCount,
          2,
        ...
        check(
          "choosing a group records it against the instructor",
        ...
        check(
          "...and clearing it means all students",
      ...
      */
        ...
        if (outsider) {
          ...
          check(
            "an instructor who does not teach the course cannot make it a group",
            await refusal(() =>
          ...
          check(
            "...nor rename one",
            await refusal(() => asOutsider.groups.rename({ groupId: squad.id, name: "Not yours" })),
            "FORBIDDEN",
          );
          check(
            "...nor change who is in it",
            await refusal(() =>
        ...
        check(
          "...and its students stay in the cohort",
          await tx.enrollment.count({ where: { id: inside.id, status: "ACTIVE" } }),
          1,
        );
        check(
          "...and its memberships go with it",
          await tx.groupMembership.count({ where: { groupId: squad.id } }),
          0,
        );
      ...
      */
        ...
        check(
          "removing the group somebody is filtered to returns them to all students",
          (await asInstructor.groups.listForCourse({ courseId: course.id })).gradingGroupId,
          null,
        );
  ...
  */
  await inOwnTransaction(db, async (tx) => {
    ...
    check(
      "two groups in one course cannot share a name",
      await refusal(() =>
  ...
  await inOwnTransaction(db, async (tx) => {
    ...
    check(
      "a student cannot be in one group twice",
      await refusal(() =>
  ...
  check(
    "no groups survived the rollback",
    await db.courseGroup.count({ where: { name: { startsWith: "Verify " } } }),
    0,
  );
