...
check(
  "algorithm files under src/ with jest classify as algorithm",
  classifySections({
...
check(
  "short-response.md classifies regardless of location",
  classifySections({
...
check(
  "shortXresponse.md is not a short response",
  classifySections({
...
check(
  "short-response-notes.md is not the submission file",
  classifySections({
...
check(
  "a blended pull request reports both present",
  classifySections({
...
check(
  "nested src files are frontend, not algorithm",
  classifySections({
...
check(
  "a flat src file without jest is frontend, not nothing",
  classifySections({
...
check(
  "the same flat src file WITH jest is an algorithm exercise",
  classifySections({
    changedPaths: ["src/RecipeCollection.js"],
...
check(
  "an undeclared section is reported as unexpected",
  classifySections({
...
check(
  "without jest, a src file is frontend and the mismatch is surfaced",
  classifySections({
...
check(
  "test and config churn implies no section",
  classifySections({
...
check(
  "a changed file matching no rule is named, not silently dropped",
  classifySections({
...
check(
  "ignorable churn is not reported as unclassified",
  classifySections({
...
for (const path of ["SHORT_RESPONSE.MD", "short_response.md", "src/short-response.md"]) {
  ...
  check(
    `${path} is both detected and sent`,
  ...
  check(`${path} is not also frontend content`, belongsToSection(path, "coding_frontend"), false);
}
...
check(
  "a deliberately withheld deliverable is sent, not filtered",
  [promptExclusionReason("server/index.js"), promptExclusionReason("server/routes/events.js")],
  [null, null],
);

check(
  "the partition keeps work and names what it withheld",
  partitionForPrompt([
    "src/from-scratch.js",
    ".env",
...
check(
  "exclusions are summarized by reason",
  summarizeExclusions([
    { path: ".env", reason: "environment file" },
    { path: "node_modules/a/index.js", reason: "dependency tree" },
...
check(
  "evidence:tests is required for test evidence",
  hasTestEvidence({ type: "x", evidence: "tests" }),
  true,
);
...
check(
  "a section with no evidence declared expects no tests",
  resolveSectionTests({ type: "coding_frontend" }, someTests).kind,
  "not-expected",
);
check(
  "tests expected with no run at all is a fault",
  resolveSectionTests({ type: "coding_algorithm", evidence: "tests" }, []).kind,
  "run-missing",
);
check(
  "tests expected and present is evidence",
  resolveSectionTests({ type: "coding_algorithm", evidence: "tests" }, someTests).kind,
  "results",
);
check(
  "a pattern that matches nothing is a fault, not an empty suite",
  resolveSectionTests(
...
check(
  "a pattern that matches some tests narrows to them",
  resolveSectionTests(
...
check(
  "a consistent report with no facts passes",
  crossCheck(report(), noFacts).needsManualReview,
  false,
);

check(
  "items not summing to the score is caught",
  codes(crossCheck(report({ scoreEarned: 11 }), noFacts)),
  ["ARITHMETIC_MISMATCH"],
);
check(
  "a possible-total mismatch is caught",
  codes(crossCheck(report({ scorePossible: 15 }), noFacts)),
  ["ARITHMETIC_MISMATCH"],
);
check(
  "earning more than possible is caught",
  codes(
    crossCheck(
      report({
        scoreEarned: 13,
...
check(
  "a staff flag left in the report text is caught",
  codes(
    crossCheck(
      report({
...
check(
  "the flag recorded in the flags array is not itself a problem",
  crossCheck(report({ flags: ["MECHANICAL"] }), noFacts).needsManualReview,
  false,
);
// Ordinary prose that happens to contain the word must not trip it.
check(
  "the word flag in a normal sentence is fine",
  crossCheck(
    report({
...
check(
  "a report whose text contradicts its recorded score is caught",
  codes(
    crossCheck(
      report({
...
check(
  "matching text and recorded score passes",
  crossCheck(
    report({
...
check(
  "a half-credit score in the text is matched, not rounded",
  crossCheck(
    report({
      scoreEarned: 5.5,
      scorePossible: 6,
...
check(
  "a report with no score heading is not flagged",
...
check(
  "a claim matching the run passes",
  crossCheck(
...
check(
  "claiming a failed test passed is a contradiction",
  codes(
    crossCheck(
...
check(
  "a claim using the qualified Suite \u203a name form matches",
  crossCheck(
    report({
      testClaims: [
...
check(
  "the qualified form still catches a contradiction",
  codes(
    crossCheck(
      report({
        testClaims: [
...
check(
  "case and whitespace differences do not break matching",
  crossCheck(
    report({
      testClaims: [
        { testName: "  FROM SCRATCH TESTS \u203a  Fizzbuzz   Works ", claimedStatus: "failed" },
...
check(
  "a claim about a test that did not run is caught",
  codes(
    crossCheck(
...
check(
  "full marks while a test failed is a contradiction",
  codes(
    crossCheck(
      report({
        scoreEarned: 12,
        scorePossible: 12,
        rubricItems: [
          { label: "x", criterion: "algorithm", scoreEarned: 12, scorePossible: 12, note: null },
        ],
      }),
      withTests,
    ),
...
check(
  "withholding points despite a 100% pass rate is LEGITIMATE",
  crossCheck(
    report({
      scoreEarned: 6,
      scorePossible: 12,
...
check(
  "a changed protected path always routes to review",
  codes(
    crossCheck(report(), {
      tests: null,
...
*/
check(
  "low confidence is not a finding",
  codes(crossCheck(report({ confidence: "low" }), noFacts)),
  [],
);
check(
  "...so it does not hold a draft back",
  crossCheck(report({ confidence: "low" }), noFacts).needsManualReview,
  false,
);
check(
  "...while a real fault beside it does",
...
check(
  "no numeric constraints in the schema",
  ["minimum", "maximum", "exclusiveMinimum", "multipleOf"].filter((k) =>
    serialized.includes(`"${k}"`),
  ),
  [],
);
check(
  "no string length constraints in the schema",
...
check(
  "every flag in the vocabulary is accepted",
  parseGradingReport(report({ flags: [...REPORT_FLAGS] })).flags.length,
  REPORT_FLAGS.length,
);
check(
  "several flags on one section are accepted",
...
check(
  "the retired MECHANICAL_ERRORS spelling is rejected",
  retiredFlagThrew,
  "ReportValidationError",
);
check(
  "prose belongs in instructorNotes",
...
check(
  "a rubric section is sliced at the next heading",
