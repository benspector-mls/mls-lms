...
async function main() {
  ...
  const {
    approveDraft, ApprovalError, buildFeedbackMarkdown, deliveryOutcome, effectiveSection,
    statedScoreInText, undeliveredApprovalWhere,
  ...
  check("a comment on a pull request is posted",
    deliveryOutcome({ postedPrCommentId: BigInt(123) }, withPr), "posted");
  check("no comment where there is a pull request is a failure",
    deliveryOutcome({ postedPrCommentId: null }, withPr), "failed");
  check("no comment where there is no pull request is not applicable",
    deliveryOutcome({ postedPrCommentId: null }, noPr), "not_applicable");
  ...
  check("half a pull request is still nowhere to post",
  ...
  */
  check("the undelivered query requires somewhere to have posted to",
  ...
  check("submitted work with no report needs one",
    triageBucket("SUBMITTED", null, false, false, false), "needs_report");
  check("submitted work on a hand-graded assignment needs a grade, not a report",
    triageBucket("SUBMITTED", null, false, false, true), "needs_manual_grade");
  check("a report waiting to be read is ready whichever way it was made",
    triageBucket("SUBMITTED", { status: "READY" }, false, false, false), "draft_ready");
  ...
  check("a delivered grade is nobody's work",
    triageBucket("GRADED", { status: "APPROVED" }, false, false, false), null);
  check("a hand-graded submission is finished once approved",
    triageBucket("GRADED", { status: "APPROVED" }, false, false, true), null);
  check("an undelivered comment outranks every other bucket",
    triageBucket("GRADED", { status: "APPROVED" }, false, true, false), "comment_not_posted");
  ...
  check("a stale draft falls through to needing a report",
    triageBucket("RESUBMITTED", { status: "READY" }, true, false, false), "needs_report");
  check("a stale draft on a hand-graded assignment falls through to needing a grade",
    triageBucket("RESUBMITTED", { status: "READY" }, true, false, true), "needs_manual_grade");
  ...
  check("all-manual sections are manual-only",
    isManualOnly([{ grading: "manual", label: "Reflection", pointValue: 10 }]), true);
  check("a mix is not manual-only",
    isManualOnly([{ grading: "ai", type: "short_response" }, { grading: "manual" }]), false);
  ...
  check("a section with no grading mode counts as AI",
    isManualOnly([{ type: "coding_algorithm" }]), false);
...
async function handGradedLifecycle(db: typeof import("../lib/prisma").db) {
  ...
  try {
    await db.$transaction(async (tx) => {
      ...
      check("submitting is what enters the queue",
      ...
      check("the queue knows this assignment is graded by hand", queued.assignment.manualOnly, true);
      check("submitted hand-graded work needs a grade, not a report",
        row?.bucket, "needs_manual_grade");

      // --- an empty draft, refused until it is filled in -------------------
      const drafts = await asInstructor.gradingDrafts.listForSubmission({
        submissionId: submitted.id,
      });
      check("no report can be generated for it",
        [drafts.canGenerate, drafts.canGradeByHand, drafts.manualOnly], [false, true, true]);
      ...
      check("the draft has one blank section per declared section",
        blank.sections.map((s) => [s.sectionType, s.scoreEarned, s.scorePossible, s.reportMarkdown]),
      ...
      try {
        ...
        blankApproval = err instanceof Error && /no score or no feedback/.test(err.message)
          ? "refused"
          : `unexpected: ${err instanceof Error ? err.message : String(err)}`;
      }
      ...
      check("releasing records the grade",
        [released.finalScore, released.finalScorePossible, released.isComplete], [17, 20, true]);
      /*
      ...
      */
      check("delivery is not applicable rather than failed",
        [released.delivery, released.commentError], ["not_applicable", null]);

      const afterRelease = await asInstructor.gradingDrafts.listForSubmission({
        submissionId: submitted.id,
      });
      check("the review screen reports it the same way",
        afterRelease.grade?.delivery, "not_applicable");
      ...
      */
      ...
      check("a released hand-graded submission is in no bucket",
        afterQueue.submissions.find((entry) => entry.id === submitted.id)?.bucket, null);

      const triage = await asInstructor.submissions.triage({ courseId: course.id });
      check("...and is not outstanding on the triage screen",
        triage.submissions.some((entry) => entry.id === submitted.id), false);

      const gradebook = await asInstructor.courses.gradebook({ courseId: course.id });
      check("...nor in the gradebook",
        gradebook.cells.find((cell) => cell.id === submitted.id)?.bucket, null);
      ...
      check("the student sees the released grade",
        [mine?.submissions[0]?.finalScore, mine?.submissions[0]?.isComplete], [17, true]);
