...
// --- path matching -------------------------------------------------------
check("tests/ dir matches nested", matchesProtectedPath("tests/a/b.spec.js", DEFAULT_PROTECTED_PATHS), true);
check("bare tests matches", matchesProtectedPath("tests", DEFAULT_PROTECTED_PATHS), true);
check("testsomething does NOT match tests/**", matchesProtectedPath("testsomething.js", ["tests/**"]), false);
check("jest.config.js matches glob", matchesProtectedPath("jest.config.js", DEFAULT_PROTECTED_PATHS), true);
check("src/index.js is not protected", matchesProtectedPath("src/index.js", DEFAULT_PROTECTED_PATHS), false);
...
check("scores/scores.json is NOT protected", matchesProtectedPath("scores/scores.json", DEFAULT_PROTECTED_PATHS), false);
check("hooks/pre-commit is NOT protected", matchesProtectedPath("hooks/pre-commit", DEFAULT_PROTECTED_PATHS), false);
check("a routine mod-1 commit reports no tampering",
  findTamperedPaths([
...
// --- tamper detection ----------------------------------------------------
check("added test file is reported",
  findTamperedPaths([{ path: "tests/new.spec.js", kind: "added" }], DEFAULT_PROTECTED_PATHS),
  [{ path: "tests/new.spec.js", kind: "added" }]);
check("student source change is ignored",
  findTamperedPaths([{ path: "src/from-scratch.js", kind: "modified" }], DEFAULT_PROTECTED_PATHS),
  []);
check("test renamed out of tests/ reads as removed",
  findTamperedPaths([{ path: "notes/a.spec.js.bak", kind: "renamed", previousPath: "tests/a.spec.js" }], DEFAULT_PROTECTED_PATHS),
  [{ path: "tests/a.spec.js", kind: "removed", previousPath: "tests/a.spec.js" }]);

// --- package.json merge --------------------------------------------------
const template = { scripts: { test: "jest" }, devDependencies: { jest: "^29.0.0" }, type: "commonjs" };

const strict = mergePackageJson(
  template,
  { scripts: { test: "echo ok", start: "node ." }, devDependencies: { jest: "^29.0.0", lodash: "^4.0.0" } },
...
check("strict: both reported", strict.overriddenKeys.sort(),
  ["package.json#devDependencies.lodash", "package.json#scripts.test"]);

const loose = mergePackageJson(
  template,
  { scripts: { test: "echo ok" }, dependencies: { chalk: "^5.0.0" }, devDependencies: { jest: "^29.0.0" } },
...
const injected = mergePackageJson(template, { jest: { testMatch: [] }, type: "module" }, { allowStudentDependencies: false });
...
const script = buildRestoreScript(["tests/**", "jest.config.*", "package.json"],
  { workDir: "/work", templateDir: "/template", resultsDir: "/results" });
...
check("package.json excluded from blanket overlay", script.includes("rm -f /work/package.json"), false);
check("creates results dir", script.includes("mkdir -p /results"), true);
let threw = false;
try { buildRestoreScript(["../../etc/passwd"], { workDir: "/work", templateDir: "/t", resultsDir: "/r" }); }
catch { threw = true; }
check("rejects an escaping pattern", threw, true);

// --- jest parser ---------------------------------------------------------
const jestOut = JSON.stringify({ testResults: [{ name: "/work/tests/a.spec.js", assertionResults: [
  { ancestorTitles: ["sumTo"], title: "adds", status: "passed", duration: 4 },
  { ancestorTitles: ["sumTo"], title: "handles 0", status: "failed", duration: 2, failureMessages: ["Expected 0, got 1"] },
  { ancestorTitles: [], title: "todo", status: "pending" }]}]});
const parsed = parseResults("jest-json", jestOut);
check("jest counts", { t: parsed.total, p: parsed.passed, f: parsed.failed, s: parsed.skipped }, { t: 3, p: 1, f: 1, s: 1 });
...
// --- pytest parser -------------------------------------------------------
const pyOut = JSON.stringify({ tests: [
  { nodeid: "tests/test_loops.py::test_sum", outcome: "passed", duration: 0.012 },
  { nodeid: "tests/test_loops.py::test_edge", outcome: "failed", call: { longrepr: "assert 0 == 1" } }]});
...
check("empty suite pass rate is null", computePassRate({ total: 0, passed: 0, failed: 0, skipped: 0, tests: [] }), null);

// --- parse failure is not test failure ----------------------------------
let parseThrew = "";
try { parseResults("jest-json", "not json"); } catch (e) { parseThrew = errName(e); }
check("unparseable output throws ResultParseError", parseThrew, "ResultParseError");
try { parseResults("jest-json", ""); } catch (e) { parseThrew = errName(e); }
...
const resolved = resolveRunner({ runnerPreset: "node-jest", runnerConfig: { e2bTemplate: "custom", timeoutMs: 5000 } })!;
...
let unknownThrew = "";
try { resolveRunner({ runnerPreset: "nope", runnerConfig: null }); } catch (e) { unknownThrew = errName(e); }
