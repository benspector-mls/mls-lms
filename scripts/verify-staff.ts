...
async function main() {
  ...
  check("an unused link inside its window is open",
    invite.inviteState({ redeemedAt: null, expiresAt: future }, now), "open");
  check("an unused link past its window has expired",
    invite.inviteState({ redeemedAt: null, expiresAt: past }, now), "expired");
  /*
  ...
  */
  check("a used link reads as used even after it would have expired",
    invite.inviteState({ redeemedAt: past, expiresAt: past }, now), "redeemed");
  check("expiry is exclusive at the boundary, so a link is dead on the second it names",
    invite.inviteState({ redeemedAt: null, expiresAt: now }, later), "expired");
  check("only an open link is usable",
    [
  ...
  check("redeeming raises a student to instructor",
    invite.raiseRole("STUDENT", "INSTRUCTOR"), "INSTRUCTOR");
  check("...leaves an instructor an instructor",
    invite.raiseRole("INSTRUCTOR", "INSTRUCTOR"), "INSTRUCTOR");
  ...
  check("...and never demotes an admin",
    invite.raiseRole("ADMIN", "INSTRUCTOR"), "ADMIN");

  check("an invitation expires within the stated window",
  ...
  */
  ...
  `);
  check("the browser can only write its own display name and avatar",
    writable.map((row) => row.column_name), ["avatar_url", "display_name"]);
  ...
  const inviteGrants = await db.$queryRawUnsafe<{ privilege_type: string }[]>(`
    SELECT privilege_type
     ...
     WHERE table_schema = 'public'
       ...
       AND grantee IN ('anon', 'authenticated')
  `);
  check("the browser has no access to instructor invitations at all",
  ...
  `);
  check("...with row level security on, so a later grant still denies by default",
  ...
  */
  ...
  try {
    await db.$transaction(async (tx) => {
      ...
      check("an unknown token previews as nothing",
        await asStudent.staff.previewInvite({ token: "not-a-real-token" }), null);
      ...
      check("...and the role is actually written",
        (await tx.profile.findUnique({
          where: { id: student.id },
          select: { role: true },
        }))?.role,
        "INSTRUCTOR");
      check("...and who used it is recorded",
      ...
      */
      check("a second person cannot use the same link",
        await refusal(() => asAdmin.staff.redeemInvite({ token: created.token })),
        "PRECONDITION_FAILED");
      check("...while the person who used it can open it again",
        (await asStudent.staff.redeemInvite({ token: created.token })).alreadyRedeemed, true);
      ...
      check("an expired invitation is refused",
        ...
        "PRECONDITION_FAILED");
      check("...and reads as expired rather than as missing",
        (await asStudent.staff.previewInvite({ token: stale.token }))?.state, "expired");
      ...
      /*
        ...
        how they got in.
      */
      check("a used invitation cannot be deleted",
      ...
      check("an instructor cannot list staff",
        await refusal(() => asInstructor.staff.people()), "FORBIDDEN");
      check("an instructor cannot list invitations",
        await refusal(() => asInstructor.staff.invites()), "FORBIDDEN");
      check("an instructor cannot generate an invitation",
        await refusal(() => asInstructor.staff.createInvite()), "FORBIDDEN");
      check("an instructor cannot delete one",
        await refusal(() => asInstructor.staff.revokeInvite({ inviteId: created.id })),
        "FORBIDDEN");
      // The escalation this whole file exists to refuse.
      check("an instructor cannot make themselves an admin",
        await refusal(() =>
          asInstructor.staff.setAdmin({ profileId: student.id, admin: true })),
        "FORBIDDEN");
      check("an instructor cannot make anybody else one either",
      ...
      check("...and now there are two",
        (await asAdmin.staff.people()).adminCount, 2);
      check("promoting again changes nothing",
        (await asAdmin.staff.setAdmin({ profileId: student.id, admin: true })).role, "ADMIN");
      ...
      */
      check("only one admin is left at this point",
        (await asAdmin.staff.people()).adminCount, 1);
      check("revoking the last admin is refused",
        await refusal(() => asAdmin.staff.setAdmin({ profileId: admin.id, admin: false })),
        "PRECONDITION_FAILED");
      check("...and they are still an admin",
        (await tx.profile.findUnique({
          where: { id: admin.id },
          select: { role: true },
        }))?.role,
      ...
      if (stillAStudent) {
        check("a student cannot be promoted directly",
          await refusal(() =>
      ...
      check("staff are listed and students are not",
        people.people.every((row) => row.role === "INSTRUCTOR" || row.role === "ADMIN"), true);
      check("the caller is marked as themselves",
        people.people.find((row) => row.id === admin.id)?.isYou, true);
  ...
  // ---- Nothing survived ---------------------------------------------------
  check("the admin is still an admin",
