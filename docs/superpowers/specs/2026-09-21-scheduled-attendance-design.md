...
**The printable page** is a new route, `app/present/attendance/[programId]/codes/page.tsx`, beside the projector page and under the same instructor-only loader. It lists the first fourteen coming days from `upcoming`, one row per day: the weekday, the date, the four-digit code in large type, and "class starts 9:30 AM, code works 7:30 AM to 5:30 PM". Above the table, one sentence for the person holding it: give out today's code only, and a code the instructor replaces or a day the instructor removes makes that row wrong. The page uses print styles so that the browser's print command produces one clean sheet.
