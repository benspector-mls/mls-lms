...
async function Day({ params }: { params: Promise<{ programId: string; date: string }> }) {
  ...
  const grid = await queryClient.fetchQuery(
    trpc.attendance.grid.queryOptions({ programId, day: parsed.data }),
  );

  if (grid.isToday) redirect(attendanceHref(programId));
