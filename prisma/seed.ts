...
async function main() {
  ...
  */
  ...
  /*
    ...
    instead: a program is identified by its name and its matriculation, which is what
    `@@unique([name, matriculation])` says, and a course by its name within that program.
  ...
  */
  ...
  // the seeded matriculation is a real-looking term. It is also what makes the suggested short
  // name come out as `fse-f26` rather than `fse-cohort-test`.
  const MATRICULATION = "Fall 2026";
  const COURSE_NAME = "Fullstack Software Engineering";

  const program =
    (await prisma.program.findFirst({
      where: { name: PROGRAM_NAME, matriculation: MATRICULATION },
    })) ??
  ...
  console.log(`Program: ${program.name} (${program.matriculation}) — ${program.id}`);
  ...
  const course =
    ...
    (await prisma.course.create({
      data: {
        ...
        slug: suggestCourseSlug({ courseName: COURSE_NAME, matriculation: MATRICULATION }),
        publishedAt: new Date(),
      },
    }));
  console.log(`Course: ${course.name} (${program.matriculation}) — ${course.id}`);
