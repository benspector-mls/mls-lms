...
- **A `Program` is a matriculation, and it owns everything that is the same for every course of that year** — one roster, one set of attendance days, one division of the roster between instructors, one join link. A course owns the work. See [the data model](#data-model).
- **"Cohort" means a division of a roster and nothing else.** It does not mean a course and it does not mean a term. The term is `Program.matriculation`. See [cohorts](#cohorts-and-grading-a-portion-of-a-roster).
- **A fellow who repeats, repeats the matriculation.** They enroll in the next one and take every course in it; there is no way to join a single course of a program. That is what gives the roster, the cohorts and the attendance day one answer each.
...
- **A course that has not started is unpublished, not unenrolled.** Being on the roster makes somebody a student of every course of the matriculation, so `Course.publishedAt` is what keeps a course beginning in March off a screen in September. It is separate from archiving, because a course that has finished must not read as one that never began.
...
- **A program link grants a matriculation; only an admin grants a role.** The instructor link admits an account that is already staff, and refuses one that is not rather than promoting it. See [instructing one matriculation](#instructing-one-matriculation-and-being-named-on-its-courses).
- **Instructing a matriculation grants the work; being named on a course does not grant anything.** Every instructor of a program authors, grades, and reads every fellow's work in every course of it. A `CourseInstructor` row decides whose name is on a course and who becomes a repository collaborator.
- **Deleting is permanent, owner-only, and reachable only through archiving**, at both scales. Deleting a course leaves the roster standing; deleting a matriculation does not. The confirmation states what would go and asks for the half of the name that is unique to it — a course's short name, a matriculation's term. See [deleting a course, and deleting a matriculation](#deleting-a-course-and-deleting-a-matriculation).
...
- **Owning a matriculation and instructing one differ.** The owner archives it, decides who instructs it and who is named on which course, and can only be removed by themselves. See [who owns a matriculation](#who-owns-a-matriculation).
...
- **Instructor procedures also check that the caller instructs *this* matriculation**, because the role alone would let one year's instructor read another's. `courseProcedure` resolves a course's program and asks the same question, in one query.
...
- **Three questions deliberately use neither**, each with a comment saying why: an instructor refused enrollment as a fellow of their own matriculation, reading a remembered `cohortId` off the instructor row, and grading triage — which answers "not yours" with an empty pile rather than a refusal.
...
**A `Program` is a matriculation — "Software Engineering Fellowship, Fall 2026" — and it owns everything that is the same for every course of that year.** One roster, one set of attendance days, one division of that roster between instructors, one join link, one instructor link. A course owns the work: its units, its assignments, its gradebook, and the team sets its projects are handed in through.

The word **cohort** means the division of a roster and nothing else. It does not mean a course and it does not mean a term; the term is `Program.matriculation`.
...
**`roster_entries`** is who is expected in a matriculation, written before they have an account — see [getting fellows into a program](#getting-fellows-into-a-program). Three CHECK constraints carry what Prisma cannot say: an entry needs at least one of `github_username` and `email`, both are stored equal to their own lowercase, and `claimed_by_id` and `claimed_at` are set together or not at all.
...
- **A fellow joins a matriculation, not a course**, and one enrollment admits them to every course of it. That is the duplication the program above the course removed: identical rosters were being entered once per course of a term, and a fellow taking four courses held four links saying the same thing.
...
- **An enrollment that already exists outranks the roster.** `join` and `preview` both answer from the enrollment before consulting the list. An entry can be absent for reasons that say nothing about belonging: staff are exempt, a claimed entry can be tidied, and a matriculation may predate its own list.
...
- **It stayed on the course when the roster moved up to the program**, and that is what lets two courses of one matriculation both hold an assignment called `project-1`. Moving the slug to the program would have made their generated names collide.
...
- **Suggested from the course name and the matriculation together, then editable once.** "Data Science" in "Fall 2026" offers `data-science-f26`; "Fullstack Software Engineering" offers `fse-f26`. The form follows both until somebody edits the slug, tracked as "have they touched it" rather than by comparing values.
...
- **Unique across every course, archived ones included**, because their repositories still exist. The constraint is the guarantee; naming both halves only makes collisions rare. Two courses of one matriculation whose names abbreviate the same way are a named refusal rather than a constraint error.
...
| `assertProgramMember`    | the same, one scope up                                    | a matriculation's screens, a fellow's own attendance        |
| `assertActiveInProgram`  | active fellows of a matriculation                         | checking into an attendance day                             |
...
### Instructing one matriculation, and being named on its courses

- **`programs.instructorToken`** is a second link per matriculation, shown on its settings screen. Opening it and pressing the button writes a `ProgramInstructor` row with `isPrimary: false`.
...
- **Reusable**, because it is bounded by the role check rather than by being spent, and a matriculation gains instructors across a year. `regenerateInstructorToken` is the control.
...
- **Removing the last instructor is refused**, the same shape as revoking the last admin: every authoring procedure gates on `ProgramInstructor`, so a matriculation with no rows there cannot be authored in or graded by anybody.
...
### Who owns a matriculation

Everybody who instructs a program authors in every course of it, reads every submission, and approves grades. **The owner also decides whether the matriculation is archived, who else instructs it, and who is named on which course** — the actions that reach past the person performing them.

- **The owner cannot be removed by anybody else.** They can leave on their own account.
- **The owner can hand the matriculation on.** `transferOwnership` moves it to another of its instructors; leaving afterwards is the ordinary removal. Without it, the rule above would read as "whoever set this up runs it forever".
- **Only the owner archives, and only the owner reopens.** One mutation with a boolean, so it is one gate. A co-teacher finds an archived matriculation in their list, reads all of it, and cannot bring it back.
...
- **The fallback is what makes a deleted account safe.** `ProgramInstructor` cascades on the profile, so deleting an owner's account would otherwise leave a matriculation with instructors, none of whom could archive it or remove anybody. Nothing here deletes a profile — that is a database action taken by hand — which is exactly why the rule has to hold with nobody there to invoke it. It also covers an owner who leaves without transferring, and the procedure says who inherited.
- **An admin acts as owner on every matriculation**, as the recovery path for an owner who left without handing it on.
...
### Deleting a course, and deleting a matriculation

Two destructive operations at two scales, and what each leaves standing is the difference worth reading.

- **Deleting a course** takes its units, assignments, submissions, grading drafts, sections, test runs, team sets and the rows naming its instructors. **It leaves the roster.** Enrollments, cohorts and attendance belong to the matriculation, so deleting one course of four leaves everybody exactly where they were, in the other three — which is why `courses.removalImpact` reports the roster size as something that *stays* rather than counting it as a loss.
- **Deleting a matriculation** takes all of that, once per course, and the roster, the cohorts and the whole attendance record with it. It is the largest destructive operation here.
...
**Both take a program and a course**, because the enrollment condition is a program fact and the course scope is what it used to supply. A caller who forgot the course would silently widen a screen from one course to every course of the matriculation, with nothing thrown — so the fragment carries it rather than leaving it to each caller.
...
- **A gradebook and a roster want opposite things, so they are two reads at two scopes.** `programs.roster` returns every enrollment of the matriculation with its status and its cohort, so it can offer to restore. `courses.gradebook` returns `activeEnrollments` and `removedEnrollments` as complements for one course, and the grid draws two tables with only the active ones in any count.
...
- **The number counts across the deployment, not per matriculation**, because a profile belongs to neither. The same test student can sit on several rosters, and must, since `github_username` is unique. Reusing one is safe: the repository name carries the course slug. The roster's dialog offers a new one or one that already exists.
...
- **A cohort is a named division of one matriculation's roster and nothing else.** It has no instructor, grants no permission, and decides nothing about who may grade. An instructor picks one from the filter on grading triage, an assignment's queue, the gradebook, or the curriculum list, and those four screens narrow to it. The overlap stops because the piles stop overlapping, not because anything is refused — a co-teacher covering for somebody else must still be able to approve their drafts.
- **It belongs to the program, and that is what the change bought.** Dividing a roster between co-teachers was never a per-course fact, so rebuilding it inside every course of a term produced the same answer several times over. One placement now narrows every course an instructor works in, and `ProgramInstructor.cohortId` remembers which cohort they are grading — one value for the matriculation rather than one per course.
...
- **`code_secret` is never selected into a payload.** The same rule as `Program.joinToken` and sharper — that one admits somebody to a matriculation, this one lets them mark themselves present from bed. `verify:attendance` walks the fellow-reachable payloads looking for the key, because the failure would be silent and total.
...
- **Absence is derived while a session runs and written down when it ends.** Ending it inserts an `ABSENT` / `FINALIZED` row for every active enrollment that has none, in the same transaction, `skipDuplicates` letting `@@unique([sessionId, enrollmentId])` decide who already had one. Starting a session also ends and finalizes any earlier open session of that matriculation, so tomorrow closes yesterday's books. **A CHECK constraint makes a `FINALIZED` row that says anything but `ABSENT` unrepresentable** — it would be the application asserting attendance on the strength of no code typed and no decision made, which is the one claim this table must never support, because it is the claim a stipend is paid against.
...
| `/courses`                                                   | A fellow's courses, across every matriculation                          |
...
| `/programs`                                                  | Every matriculation the caller belongs to. An instructor's way out      |
| `/programs/[programId]/attendance`                            | A fellow's own attendance record for one matriculation                  |
...
| `/instructor/programs/[programId]/settings`                  | The matriculation itself: its courses, who instructs it, its lateness rule, how it ends |
| `/instructor/programs/[programId]/students/[studentId]`      | One fellow across the whole matriculation — the person, not the work    |
...
| `/join/[token]`                                              | Where a matriculation's join link lands, and says whether it is theirs  |
| `/invite/[token]`                                            | Where an instructor invitation lands                                    |
| `/teach/[token]`                                             | Where a matriculation's instructor link lands                           |
...
- **Your week**, a strip above everything else: one row per matriculation, this week's mornings as squares, the term's attendance beside them, and the check-in code on the mornings a session is open. One row rather than one per course, because there is one morning.
...
The instructor sidebar is two groups. **A program group** of three — Attendance, Roster, Settings — and **the matriculation's courses**, each opening to five of its own: Triage, Gradebook, Curriculum, Teams, Settings. Each group is in the order it is reached for. Attendance leads the program's because it is the only item in either touched at a fixed time every morning; Triage leads a course's because "what is waiting on me" is the question an instructor opens this application to ask.

- **The split is the model made visible.** The roster, the mornings, the cohorts and the instructors are the same for every course of a matriculation, so they are one group above; the work is per course, so it is a course's own. Before the program existed, attendance and the roster sat in the course group and were rebuilt inside every course of a term.
- **The program's five views became three, and the two that went were sections rather than screens.** Placing fellows in cohorts is a thing done *to* the roster, so it is a tab on it; who instructs a matriculation is a fact about it, so it is a card on its settings. Each had an address of its own while the question was whether it was a screen, and running it answered them — five sidebar items were five doors onto three rooms, and a menu that long stops being read. What decided each was where the question is *asked*: "who has nobody grading them" is asked while reading the roster, and "who runs this year" is asked beside its term and its lateness rule.
...
- **`programOfCourse` is what fills the program group in on a course address.** It reads the matriculation off `courses.listMine`, which the sidebar already holds, so it is a lookup rather than a request — and without it the program group would vanish on the screens an instructor uses most, taking attendance with it.
- **A fellow's sidebar is one group per matriculation, and attendance sits beside the program's name.** The heading is the program and its term; under it, Attendance first and then that matriculation's courses, each linking straight into its coursework. Attendance moved up out of the courses because there is one morning: three courses meeting on a Tuesday used to mean three attendance screens saying the same thing. The courses are flat now — their sub-menu existed only to reach attendance from inside one. There is no "My courses" item pointing at the list; `/courses` is still a real screen, reached by the breadcrumb and by `/`, and it is what is offered to somebody with no enrollment yet. Archived matriculations and ones a fellow was removed from stay in the list, labelled with the same words the cards use, sorted after the current ones.
- **The two switchers are instructors-only, and the asymmetry is a decision rather than an omission.** An instructor works matriculations whose screens are identical and which accumulate over years, so a switcher trades one click for a sidebar of fixed height. A fellow is in one matriculation at a time and lands on `/dashboard`, which names none — a header switcher would greet them every morning with a control pointing at something the screen is not about, and guessing is what the note above says went wrong.
...
- **Only the current matriculation's courses are listed**, which is what keeps it readable: courses are named for what they teach and every year runs the same ones, so an unscoped list would hold four rows reading "Fullstack Software Engineering" that nothing on the row could tell apart. Reaching another year is the program switcher in the header.
- **All programs sits in its own group at the top, separated by a rule**, because everything below is scoped to one matriculation or one of its courses and this is the way out of all of them.
- **Switching keeps the view** rather than returning to a front page, in both directions. `sameViewInCourse` and `sameViewInProgram` decide, each knowing all of its own group — a view missing from one falls through to settings rather than failing, which is why every item of both is checked by `verify:enrollment`. Two things deliberately do not travel: **one day of attendance**, because the other matriculation may not have met that day and landing on an empty screen offering to record a morning that never happened is worse than landing on today; and **a fellow's record**, because a person is on one roster or another.
...
- **`/profile` belongs to a person rather than to a matriculation**, which is why it sits at the top level and why a student and an instructor reach the same address. It is reached from the account menu at the foot of the sidebar and nowhere else. There is no role gate, because `me` and `updateDisplayName` are both scoped to `ctx.user.id`.
...
| Who may join a matriculation         | Per-program roster, checked in `enrollments.join` | `lib/courses/roster.ts`    |
...
- A fellow can read the list but call none of the writes, and an instructor who does not instruct the matriculation cannot either.
...
**Cohorts.** Most of `verify:cohorts` is not about the cohorts table. What has to hold is that **filtering to one cohort narrows grading triage, an assignment's queue, the gradebook, and the curriculum list to the same set of people** — and, since a cohort belongs to the matriculation, that one placement does it in every course of that year.
...
- Around them: a cohort from another matriculation matches nothing rather than everything, No cohort agrees with the picker's own figure, a removed fellow keeps their cohort and stays out of the pile until restored, choosing a cohort is remembered and deleting it returns the instructor to all fellows, and a fellow can call none of it while an instructor who does not instruct the program cannot either.
- **Two checks are the partition itself, and both replaced something.** Placing a fellow in a second cohort *moves* them out of the first, where the old grading group was a many-to-many and the check was that they could be in two at once. And the composite key `(cohort_id, program_id)` refuses another matriculation's cohort, which `groups.setMembers` used to validate by hand — the key made the validation rule unnecessary rather than porting it.
...
**Getting onto a roster, and the courses inside a matriculation.** `verify:enrollment` covers it through the callers inside a rolled-back transaction. Who instructs a matriculation and who owns it is `verify:programs`, below.
...
- **One enrollment admits them to every published course of the matriculation**, opened one at a time rather than asserted over a list, because the claim is that each call succeeds and a predicate over an array would be satisfied by an array of promises.
