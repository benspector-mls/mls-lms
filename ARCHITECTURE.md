...
- **The column holds one of three things, decided by what the submission is.** A document the browser can show (`previewKindOf`, the same function the preview itself asks, so the two cannot disagree about what can be embedded); the address a student handed in; or the working behind the score — the rubric breakdowns and the suite output. Never two of them, because the kinds decide it: work collected as a file has no pull request to run a suite against and no report a model wrote, and work collected as a pull request has no document to embed. A submission with none of the three — a `.docx`, a repository with no run and no report yet — stays in one column rather than splitting to show an empty half.
- **The working stacks after the grade; the work stacks before it.** On a narrow screen an instructor reads the feedback the student will read and then scrolls to what backs it up, which is the order this screen has always had, so the evidence column carries `order-last` below the breakpoint. A document is the opposite: it is the work, and it is read first. Both columns are placed explicitly with `col-start`/`row-start` rather than falling into the grid in order, which is what lets one piece of markup stack in one order and split in another.
- **Each column scrolls itself, and the pane stops scrolling once it splits.** Two columns of different lengths cannot share one scrollbar: the shorter one runs out and then sits there while the longer one goes on, and its last card is left below the fold with nothing that will bring it up — which is what a rubric of ten questions beside a one-section report does.
- **The height comes from `h-full` on the grid, never from a `calc` against the viewport.** The pane's height is already known; naming it as `100svh` minus the chrome above it is a guess that is wrong on some screen, because what sits above the pane differs between the queue, the student record, and grading mode. Too small and the column ends early; too large and its last card is unreachable.
