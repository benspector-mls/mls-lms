...
**Calibration grades with the answer key, because production does.** The key carries a per-question "Look for" list naming the terminology and points each answer has to reach, the hand-written reports these samples are compared against were marked against it, and withholding it asked the model to infer a standard that was written down. `scripts/calibrate.ts` names the short response solution directory specifically rather than the assignment's whole answer key folder, which also holds the frontend solution — five JavaScript and CSS files a short response prompt cannot use and is billed for on every run.

Below is five runs of `claude-sonnet-5` at `high` effort, against the defect-count technical bands.

|                            | Pair 1 (exemplar)  | Pair 2 (held out)  | Pair 3 (held out)  |
| -------------------------- | ------------------ | ------------------ | ------------------ |
| Instructor                 | 12/15              | 12/15              | 11/15              |
| Model                      | 12 ×5              | 12, 12, 13, 12, 13 | 11, 11, 11, 11, 12 |
| Completion decision agrees | —                  | 5 of 5             | 4 of 5             |

- **Two rubric findings produced that agreement, and both were found by disagreement rather than by reasoning.** The first: a required item *absent* is a defect, which is what the instructor deducted for on pair 3's Question 1 — a correct explanation of block scope that never names it. The second: **the "Look for" list is a floor rather than a ceiling.** Supplying the key alone made pair 2 worse, because the model treated a satisfied checklist as sufficient and stopped scoring; pair 2's Question 2 covers every required item and then adds a false claim, that a private field returns a copy. Coverage and surplus are separate failures and the bands have to score both.
- **What remains is an intermittent single point on the technical criterion**, in the generous direction, on one question of one pair in about a third of runs. It is no longer systematic, and the exemplar reproduces exactly on every run, which is what says the pipeline itself is steady.
- **A false completion is the failure that matters, and it is now rare rather than certain.** Every run of both tiers once marked pair 3 complete when the instructor marked it incomplete; one run in five does now. A wrongly-incomplete grade is disputed by the student and corrected, while a wrongly-complete grade is appealed by nobody, so this is the number to watch.
- **Pair 3 shares its four questions with the exemplar**, and its report differs from the exemplar's in exactly one cell. The model has been observed anchoring to the exemplar's scores and saying so in its instructor notes, which means the sample report in the prompt teaches scores as well as shape. A fourth pair on different questions would test the rubric more honestly than pair 3 can.
...
- **The tier comparison predates all of this.** `claude-opus-5` was measured against the earlier bands with no answer key, where it agreed on 2 of 10 completion decisions and ranked the two held-out pairs in the wrong order. Both the rubric and the prompt have changed since, so that result says nothing about how the tiers compare now, and a tier decision that needs defending should re-run both.
