...
- **The left column is sticky and the grade column scrolls**, in one scroller rather than two. `items-start` is what makes that possible — a column stretched to the grid row's height has nothing to stick to. It carries a height of its own as well, so a long suite output scrolls beside the report rather than taking the report off the screen.
