...
- **`programs.instructorToken`** is a second link per matriculation, shown on its instructors screen. Opening it and pressing the button writes a `ProgramInstructor` row with `isPrimary: false`.
...
| `/instructor/programs/[programId]/roster`                    | Who is expected, who has joined, the join link, and each fellow's cohort |
| `/instructor/programs/[programId]/cohorts`                   | How the roster is divided among its instructors                         |
| `/instructor/programs/[programId]/instructors`               | Who instructs it, who is named on which course, and who owns it         |
| `/instructor/programs/[programId]/settings`                  | The matriculation itself: its courses, its lateness rule, how it ends   |
...
### Two scopes, ten views, ten addresses

The instructor sidebar is two groups. **A program group** — Attendance, Roster, Cohorts, Instructors, Settings — and **a course group** — Triage, Gradebook, Curriculum, Teams, Settings. Each group is in the order it is reached for. Attendance leads the program's because it is the only item in either touched at a fixed time every morning; Triage leads the course's because "what is waiting on me" is the question an instructor opens this application to ask.

- **The split is the model made visible.** The roster, the mornings, the cohorts and the instructors are the same for every course of a matriculation, so they are one group above; the work is per course, so it is another below. Before the program existed, attendance and the roster sat in the course group and were rebuilt inside every course of a term.
...
- **The course switcher lists only the current matriculation's courses**, which is what keeps it usable: courses are named for what they teach and every year runs the same ones, so an unscoped list would hold four rows reading "Fullstack Software Engineering" that nothing on the row could tell apart. Reaching another year is the program switcher above it.
