...
- **A score is written after a pause in the typing, and a feedback box is opened at once.** The round replaces the blank form with the editor, so the boxes are new ones: waiting for a pause keeps that swap out of the middle of a number being typed, and a box asked for by a click is not offered until the round holding it exists. Which boxes are open is remembered above the cards, in `FeedbackBoxes`, because the card that owns a box is rebuilt around the new round.
