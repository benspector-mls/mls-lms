...
- **Removing a fellow and archiving a course make lists go quiet; they never take work back.** A removed fellow keeps reading their feedback; an archived course stays readable, and stays in the course list, labelled. Neither can hand anything new in. See [a removed student's work](#a-removed-students-work).
...
| `/courses`                                                   | A fellow's courses, across every program                          |
...
- **The breadcrumb is program, then course, then the view.** "Software Engineering Fellowship (Fall 2026) › Fullstack Software Engineering › Gradebook". The program carries its term because a program runs every year under the same name; the course carries only its own name, since repeating the term in one trail would say the same thing twice. Parenthesised rather than the switcher's middot, since a trail already separates its steps. It costs no fetch: `courses.listMine` and `programs.listMine` are what the breadcrumb reads. Both are plain text rather than links, because neither has a home for one to point at.
...
- **An archived course takes no copies**, the same rule as a fellow joining a program or an instructor being added to one. Archived courses are in the course list, so one is a thing somebody can be looking at when they reach for a copy.
