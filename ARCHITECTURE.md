...
- **Clicking a course keeps the view**, which is the one property the picker had worth keeping — from course A's gradebook, course B's row goes to *its* gradebook. `sameViewInCourse` decides, and an address that cannot travel lands on settings rather than on an id the other course does not have.
...
- **Switching keeps the view** rather than returning to a front page, in both directions. `sameViewInCourse` and `sameViewInProgram` decide, each knowing all of its own group — a view missing from one falls through to settings rather than failing, which is why every item of both is checked by `verify:enrollment`. Two things deliberately do not travel: **one day of attendance**, because the other program may not have met that day and landing on an empty screen offering to record a morning that never happened is worse than landing on today; and **a fellow's record**, because a person is on one roster or another.
