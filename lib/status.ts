 ...
 * Modules.
 *
 * `moduleTag` is a directory name in the answer-keys repository — "mod-4-dom" — which
 * is the right key to group by and the wrong thing to show a student.
 */

const MODULE_TAG_PATTERN = /^mod-(\d+)(?:-(.+))?$/;

/** Words that are initialisms rather than lowercase words, so title casing skips them. */
const INITIALISMS = new Set(['js', 'sql', 'dom', 'html', 'css', 'oop', 'api', 'ui', 'cli']);

/** "mod-4-dom" becomes "Module 4 · DOM". Anything unrecognized is shown as written. */
export function moduleLabel(tag: string): string {
  const match = MODULE_TAG_PATTERN.exec(tag);
  if (!match) return tag;

  const [, number, slug] = match;
  if (!slug) return `Module ${number}`;
  ...
  return `Module ${number} · ${words.join(' ')}`;
}

/**
 * Module tags are strings, so sorting them as strings puts "mod-10" before "mod-2".
 * Compare the numbers as numbers.
 */
function compareModuleTags(a: string, b: string): number {
  const aNumber = MODULE_TAG_PATTERN.exec(a)?.[1];
  const bNumber = MODULE_TAG_PATTERN.exec(b)?.[1];
...
/**
 * A comparator that puts modules in the order the course declares.
 *
 * `moduleStructure` is the cohort's own sequence and is authoritative where it covers a
 * tag. An assignment tagged with something not in that list still has to appear
 * somewhere, so it sorts after the declared modules by number.
 */
export function moduleOrder(moduleStructure: string[]): (a: string, b: string) => number {
  const declared = new Map(moduleStructure.map((tag, index) => [tag, index]));

  return (a, b) => {
    const aIndex = declared.get(a);
    const bIndex = declared.get(b);

    if (aIndex != null && bIndex != null) return aIndex - bIndex;
    if (aIndex != null) return -1;
    if (bIndex != null) return 1;
    return compareModuleTags(a, b);
  };
}
