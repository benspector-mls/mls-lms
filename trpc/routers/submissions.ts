 ...
 * and nobody has approved belongs in one of these, so the buckets together are the whole
 * of what stands between the instructor and being caught up. `needs_report` is usually
 * the largest of them — reports are generated by the instructor, not at each other
 * students' pushes, so having no report yet is the ordinary state of new work rather
 * than an anomaly.
 *
 * Decided here rather than in the interface so the count on a card and the rows inside it
 * can never come from two different readings of the same data.
 */
function triageBucket(
  ...
  */
  ...
  /*
    ...
    arrives and where a "generate reports for everything pending" run would find its
    subjects.
