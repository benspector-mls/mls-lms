 ...
 *
 * **A button rather than joining on arrival**, the same as the student join screen and for the
 * same reason: opening a link is not consent to take on a cohort, and this is the one screen
 * where which cohort it is can be said.
 *
 * **The ineligible case is answered here rather than by a failed button.** A student account
 * cannot be made staff from a course link — that would be a second path to staff access with no
 * admin involved — so the screen says what is actually needed instead of offering an action
 * that is going to be refused. The procedure refuses it regardless; this only decides whether
 * somebody is invited to try.
 */
export function AcceptCoTeach({
  token,
  preview,
}: {
  token: string;
  ...
  return (
    <Shell>
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        {preview.alreadyTeaches ? (
          <>
            <p className="flex items-center gap-1.5 text-sm text-emerald-700 dark:text-emerald-400">
              <Check className="size-4" />
              You already teach this cohort.
            </p>
            <Button onClick={() => router.push(courseSettingsHref(preview.courseId))}>
              Open cohort
          ...
          /*
            ...
            Everything else on this screen is about a cohort; this is about the account.
          */
          <div className="flex flex-col gap-2">
            <p className="text-sm text-muted-foreground">
              This link adds an instructor to the cohort, and your account is not an instructor
              account.
            </p>
            <p className="text-sm text-muted-foreground">
          ...
          <p className="text-sm text-muted-foreground">
            This cohort is archived, so it is not taking new instructors.
          </p>
        ) : (
          <>
            <p className="text-sm text-muted-foreground">
