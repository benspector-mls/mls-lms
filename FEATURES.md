...
**A fellow is in at most one cohort**, so the screen is a card per cohort with that cohort's fellows listed inside it in alphabetical order. The first card is **No cohort**, and it is always there, because somebody who joined by the link in October and was never placed is invisible to every instructor working a cohort — this is the card that names them. Each cohort card carries how many fellows are in it, a **Rename** and a **Remove**, and every fellow listed has a **Move to** that sends them to another cohort. Every change is saved as you make it. Removing a cohort puts its fellows back in No cohort, and **Reset** does the same for every cohort at once, leaving the cohorts themselves in place.
...
**Teams** on a course's sidebar is where they live. Make a set — "Unit 3 project teams" — and then **Create teams** inside it. Either ask for a number of empty teams and place each fellow by hand, or **Distribute evenly**: say how many fellows a team should have, tick **Keep cohorts together** if each cohort should be teamed among itself, and the screen works out how many teams that makes, makes them, and deals everybody onto them at random. It never makes a team of one — 19 fellows in twos is eight pairs and one three; 5 in threes is a three and a two. Press it again to reshuffle: it reuses the teams already there, adds only what it needs, and tells you if any are left empty. Make one set per project: the same fellows divided differently for each, with an earlier project's arrangements sitting in their own set rather than cluttering anything.
...
**Say when the program meets, and every day makes itself.** The Attendance screen's **Schedule** tab holds everything about which days exist. Under **Class meets**, give a first day, a last day, the weekdays it meets and what time class starts. Every meeting day from today onward becomes a session with its code already made: check-in opens two hours before class, being on time is measured from the start time, and the code stops working eight hours after that. Nobody presses anything, on any morning. Before you save, the screen says how many days it is about to make and how many it will remove.

**The two hours before class are the point.** Fellows arrive early, and until now the ones who did could not sign in until somebody opened the day — so the most reliably early people were the ones left waiting. An arrival before class counts as present, and the minute they arrived is recorded either way.

**Or press Start, as before.** A program with no schedule works exactly as it always did: press **Start check-in** on the Attendance screen and a four-digit code appears with a **Copy** button beside it. Either way, give the code out however costs you least: read it aloud, paste it into the Zoom chat, put it on your first slide, or press **Project the code** for a full-screen page to put on a projector or share into Zoom. Fellows type it on their dashboard or on their own attendance screen, already signed in as themselves.

**Remove the days you do not meet.** Under the same Schedule tab, **The program's days** is a calendar you page through month by month. A filled square is a day that was held and whose record is in, a dashed one is a day scheduled ahead, and a plain number is a day the program does not meet — so a holiday in the middle of a working week is a hole you can see. Open any day to read or correct it, or to remove that one day. **Remove days** above the calendar takes out a single day or a whole break at once: choose a first day and the last day follows it, so one Tuesday costs one field. A day somebody has already checked into is kept rather than destroyed, and the screen names it. A holiday nobody removes in time opens itself, takes no check-ins, and marks the whole roster absent — which shows up as a column of red on the term grid, and removing the day afterwards takes those absences with it.
...
**Three weeks of codes print on one sheet.** **Print the coming codes**, beside the calendar, gives a page for the people who open the building before an instructor arrives. Choose the day the sheet should start on and it lists every day the program meets from there through three weeks later, each with its four-digit code — fifteen rows for a full Monday-to-Friday stretch, fewer across a break, because the sheet prints the mornings that exist rather than a fixed count. Print at the end of a Monday, start the sheet on the Tuesday, and it is still good on the Monday three weeks after. It says to give out today's code only, and that replacing a code or removing a day makes that row wrong. It says to give out today's code only, and that replacing a code or removing a day makes that row wrong.
...
Open an assignment to get its **queue**: every fellow down one side, the work and the review pane on the other. Tabs for All, To do, and Graded. From a fellow's name anywhere you get the other axis — **one fellow's whole record in this course**, every assignment including the ones they never started. A second screen, reached from the roster, is about the *person* rather than the work: their attendance, their cohort, their GCF results, and a row per course of the program. Its **Trends** section is the last few weeks rather than the term: the last few mornings by the same rule as the attendance screen's list, when they arrive in the morning, overall and by weekday, and whether each course's work has slipped, by the same rule as the gradebook's list.
...
**Needs a conversation** sits above the Overview table, and it is the attendance screen's list of the same name, for the work. Those term-long late and missing totals hide exactly the fellow this list is for: somebody who finished every module in September and has handed nothing in for a fortnight. A fellow is listed once they have missed or handed in late 4 or more of the last 10 assignments due, or fallen short on 2 or more of their last 5 graded, and the **?** beside the heading says so. Missed and late are one count, because handing in a missing assignment turns it into a late one, and the list should not clear because somebody handed in a week late. Each row says which clause put them there and the figures behind it.
...
**Checking in** is on your dashboard, in your week's row, and on your **Attendance** screen beside the program's name in the sidebar. Either works and both want the same thing: the four-digit code your instructor gave out for today — **one code, whatever you have classes in**. The code does not change during the session, so if you arrive late you can get it from the chat or from the person next to you rather than asking your instructor to stop. Neither screen says anything at all on a day the program does not meet.

**Arriving early counts as on time.** Where the program has said when it meets, check-in opens two hours before class and you can sign in as soon as you sit down. Before then both screens say what time the box will appear, so there is nothing to do and nobody to ask. Your week also shows the days still to come as hollow squares, so next Tuesday being a class day is something you can see.
...
- **A program's calendar is a range with the exceptions removed, and the exceptions are yours to remove.** Saying the program meets weekdays from September to June is two fields and a row of checkboxes; taking out Thanksgiving, winter break and a snow day is a handful of acts across the year. A break nobody removes is a week of mornings that open themselves and mark everybody absent. That is recoverable — the days are still removable afterwards, and removing them takes the absences with them — but it is work somebody has to notice, and the term grid is where they notice it.
- **A program with no schedule is still supported, and still behaves as it always did.** Nothing forces a program onto a calendar. Leave the settings empty and a session exists because somebody pressed Start, with the same consequence as before: a morning nobody opened is indistinguishable from a morning the program did not meet.
