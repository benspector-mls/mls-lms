...
**Grading by hand** is the same screen with an empty draft, and the form is already on it: one card per section of the assignment, with an empty score box and an empty feedback box. The first thing you type into either one opens the draft and carries what you wrote onto it, so the score, **Discard this feedback** and **Approve and release** appear in the header as soon as there is a grade to release. Everything after that is identical. A section left with no score or no feedback is refused rather than released as a zero. Opening a submission and leaving it alone opens no draft, a score typed and taken back out again opens none either, and nothing can open a second draft over the one you have started.
