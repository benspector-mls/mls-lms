...
**Grading by hand** is the same screen with an empty draft. **Start grading** opens one, you write the feedback and the score, and everything after that is identical. A section left with no score or no feedback is refused rather than released as a zero. Pressing the button twice gives you back the draft you already started, not a second blank one.
